...
import { CANVAS_WIDTH, CANVAS_HEIGHT, MIN_THRESHOLD, HALF_THRESHOLD } from '../data/constants';
import { paintStrokes } from './canvasUtils';
...
export function calculateCombinedRegionCoverage(allDrawings, regionPath) {
  ...
  allDrawings.forEach(drawings => paintStrokes(combinedCtx, drawings, 'red'));
...
export function calculateRegionCoverage(drawings, regionPath) {
  ...
  paintStrokes(drawingCtx, drawings, 'red');
...
export function deriveSymptomFlags(coverage) {
  ...
  const thumbAffected = thumbDistalCoverage > MIN_THRESHOLD;

  const indexDistalCoverage = coverage['index_distal'] || 0;
  const indexMiddleCoverage = coverage['index_middle'] || 0;
  const indexAffected = (indexMiddleCoverage >= HALF_THRESHOLD) || (indexDistalCoverage > MIN_THRESHOLD);

  const middleDistalCoverage = coverage['middle_distal'] || 0;
  const middleMiddleCoverage = coverage['middle_middle'] || 0;
  const middleAffected = (middleMiddleCoverage >= HALF_THRESHOLD) || (middleDistalCoverage > MIN_THRESHOLD);
  ...
  const palmRadialAffected = palmRadialCoverage > MIN_THRESHOLD;
  const palmUlnarAffected = palmUlnarCoverage > MIN_THRESHOLD;
  const palmAffected = palmRadialAffected || palmUlnarAffected;
  const palmConfinedToUlnar = palmUlnarAffected && !palmRadialAffected;

  // Assumes back SVGs contain only the 'dorsum' region
  const dorsumCoverage = coverage['dorsum'] || 0;
  const dorsumAffected = dorsumCoverage > MIN_THRESHOLD;

  const wristCoverage = coverage['wrist'] || 0;
  const wristAffected = wristCoverage > MIN_THRESHOLD;
