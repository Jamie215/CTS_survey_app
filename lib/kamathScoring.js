...
/**
 * @typedef {Object} KamathBand
 * @property {number} min - Inclusive lower bound (-Infinity for the lowest band)
 * @property {number} max - Inclusive upper bound (Infinity for the highest band)
 * @property {string} classification
 * @property {string} colorClass - 'green' | 'yellow' | 'red'
 * @property {string} description
 * @property {string} legendLabel - Short legend text e.g. '<3: Unlikely CTS'
 */
...
/**
 * Classification bands for total Kamath-based score.
 *
 * This is the ground truth for the mapping score for classification.
 * Both calculateKamathScore (below) and the Results display
 * (KamathScoreCard) read from this array, so the legend text on screen
 * can never drift from the rule that produced the classification.
 *
 * Bands are inclusive on both ends. Ordering is low → high; the first
 * band whose [min, max] contains the score wins. If you tune a
 * boundary, change both the numeric bounds and the human-readable
 * legendLabel here — nowhere else.
 */
export const KAMATH_BANDS = [
  {
    min: -Infinity,
    max: 2,
    ...
    colorClass: 'green',
    description:
      'Based on questionnaire responses, symptoms are unlikely to be caused by Carpal Tunnel Syndrome.',
    legendLabel: '<3: Unlikely CTS',
  },
  {
    min: 3,
    max: 4,
    ...
    colorClass: 'yellow',
    description:
      'Questionnaire results are inconclusive. Further clinical evaluation or nerve conduction studies may be helpful.',
    legendLabel: '3-4: Possible CTS: Unclear',
  },
  {
    min: 5,
    max: Infinity,
    ...
    colorClass: 'red',
    description:
      'Questionnaire responses are consistent with Carpal Tunnel Syndrome.',
    legendLabel: '≥5: Classic / Probable CTS',
  },
];

/**
 * Look up the classification band for a given total score.
 * @param {number} totalScore
 * @returns {KamathBand}
 */
function bandForScore(totalScore) {
  return (
    KAMATH_BANDS.find((b) => totalScore >= b.min && totalScore <= b.max) ||
    KAMATH_BANDS[0]
  );
}
...
export const calculateKamathScore = (answers, hasNumbnessOrTingling) => {
  ...
  Object.entries(kamathQuestionWeights).forEach(([questionId, weights]) => {
    ...
    if (weights === null) {
      skippedQuestions.push({ id, answer: 'NA', reason: 'Gateway question - not scored' });
      return;
    }

    // Skip questions that require numbness/tingling if user answered No to Q1
    if (id >= 1 && id <= 7 && !hasNumbnessOrTingling) {
      skippedQuestions.push({ id, answer: 'NA', reason: 'Skipped - no numbness/tingling reported' });
      return;
    }

    if (id === 13 && answers[12] !== ANSWERS.YES) {
      skippedQuestions.push({ id, answer: 'NA', reason: 'Skipped - splint not tried' });
      return;
    }

    if (answer === undefined || answer === null) {
      skippedQuestions.push({ id, answer: 'NA', reason: 'Not answered' });
      return;
    }
